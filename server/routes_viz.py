...
def _load_scene(file_id: str, key: str, scene: int):
    """Load data and extract a single scene cube."""
    try:
        data = file_manager.load_key(file_id, key)
...
@router.get("/rgb")
async def viz_rgb(file_id: str, key: str, scene: int = 0):
    cube = _load_scene(file_id, key, scene)
    img = viz.render_rgb(cube)
    return _image_response(img)


@router.get("/colorized")
async def viz_colorized(file_id: str, key: str, scene: int = 0, channel: int = 2):
    cube = _load_scene(file_id, key, scene)
    try:
...
async def viz_error_map(
    ...
    pred_cube = _load_scene(file_id, pred_key, scene)
    truth_cube = _load_scene(file_id, truth_key, scene)
...
@router.get("/metrics")
async def viz_metrics(file_id: str, keys: str = Query(..., description="Comma-separated key names")):
    ...
    for k in key_list:
        try:
            data_dict[k] = file_manager.load_key(file_id, k)
        except KeyError as e:
            raise HTTPException(404, str(e))
...
async def viz_spectral_density(
    ...
    for s in source_list:
        cube = _load_scene(s["file_id"], s["key"], s.get("scene", 0))
...
async def viz_measurement(file_id: str, key: str, scene: int = 0):
    """CASSI compressed measurement: cube * shifted_mask, summed over channels."""
    cube = _load_scene(file_id, key, scene)
    try:
        mask = file_manager.get_mask()
